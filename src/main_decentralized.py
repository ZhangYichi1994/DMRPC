...
for loopTime in range(0,stepNum):
    if (selfIterationFlag == True) :
        # anotherControl = np.zeros(2 * seqSize).tolist()
        ...
        controlSeq2 = c.distributedLMPC(initialConditions, initialControl, seqSize, lstmStateSeq,1, controlSeq1)
...
plt.savefig('test_LSTM_decentralized.jpg')
plt.show()