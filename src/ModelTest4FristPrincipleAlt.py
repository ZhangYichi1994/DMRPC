# --utf-8--
# The generated data is used for LSTM test and plot
...
T10 = 471.9    # Initial temperature of tank 1 [K]
T20 = 331.9     # Initial temperature of tank 2 [K]
...
class generateDataWindow():
    ...
    def generation(self):
        ...
        # inputRecord shape: SeriesNum x Time x Feature
...
a = generateDataWindow(sampleSize, lstmWindowSize, feature, outputFeature,
                storePath, sampleInterval, loopNum)
inputRecord, outputRecord_derive, outputRecord_state = a.generation()

###### End of the Data Generation #######

digitalTwinData = np.zeros([sampleSize,lstmWindowSize,4])
...
stepNum = lstmWindowSize
stateRecord = np.zeros([stepNum, 4])       # 记录系统状态，格式：[]
...
# inputRecord, outputRecord_derive, outputRecord_state
selfIterationFlag = True    # 自迭代参数
tank1His = np.zeros([lstmWindowSize,1])
tank2His = np.zeros([lstmWindowSize,1])
for seriesLine in range(sampleSize):
    inputUse = inputRecord[seriesLine,:,:]
    outputUse =outputRecord_state[seriesLine,:,:]

    lstmStateSeq = inputUse[0:5,:]
    initialConditions = inputUse[4,0:6]
    # for loopTime in range(0, 5):
    #     # controlSeq = np.array([0,0,0,0])
    #     controlSeq = np.array(inputUse[loopTime,-4:])
    #     controlUsed = controlSeq
    #     nowControls = np.array(controlUsed)
    #     initialConditions = np.array(inputUse[loopTime, 0:6])
    #     nowConditions = np.array(initialConditions)             # 当前状态量
    ...
    for loopTime in range(5, lstmWindowSize):
        if (selfIterationFlag == True) :
            # 根据运行状态，求解优化问题，计算控制序列
            controlSeq = np.array(inputUse[loopTime,-4:])
            # 提取控制序列第一项，送入First-Principle模型或者LSTM，求解下一时刻系统状态
            controlUsed = controlSeq[0:4]         # 当前控制量
            nowControls = np.array(controlUsed)
            nowConditions = np.array(initialConditions)             # 当前状态量
            nowInput = inputUse[loopTime, :]

            selfIteration = c.nextStep(nowInput, lstmStateSeq, method='NN')
            print("LSTM output is: ", selfIteration)
            initialConditions = selfIteration  
            initialConditionsForControl = np.array(initialConditions)

            # 存数据， 前一时刻系统状态， 前一时刻控制输入， 当前时刻系统状态
            record = initialConditions[[0,1,3,4]] - c.xs
            stateRecord[loopTime, :] = record

            # 自迭代结果
            selfIterationOuput = c.nextStep(nowInput, lstmStateSeq, method='NN')
            lstmStateSeq = c.updateInputSeq(lstmStateSeq, nowInput)
            
    realOutput =  outputUse[:,[0,1,3,4]] - c.xs

    tank1 = np.hstack((stateRecord[:,[0,1]], realOutput[:,[0,1]]))
    tank2 = np.hstack((stateRecord[:,[2,3]], realOutput[:,[2,3]]))

    tank1His = np.hstack((tank1His, tank1))
    tank2His = np.hstack((tank2His, tank2))


path = f"data/model4test/tank1.csv"
np.savetxt(path, tank1His, fmt='%f', delimiter=',')
path = f"data/model4test/tank2.csv"
np.savetxt(path, tank2His, fmt='%f', delimiter=',')

    # storeVec = np.hstack((stateRecord, realOutput))
    # path = f"data/model4test/{seriesLine}_storeVec.csv"
    # np.savetxt(path, storeVec, fmt='%f', delimiter=',')
    
    # path = f"data/model4test/{seriesLine}.csv"
    # np.savetxt(path, stateRecord, fmt='%f', delimiter=',')
    # path = f"data/model4test/{seriesLine}_output.csv"
    # np.savetxt(path, outputUse[:,[0,1,3,4]] - c.xs, fmt='%f',delimiter=',')
