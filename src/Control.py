# 集中式LMPC
# 机理模型作为递推模型
# 使用“精确”方法来计算：Sontag辅助控制率、优化问题约束雅各比矩阵（参考Durand论文和代码修改）
"""
论文：
...
sampleInterval = 0.001     # 采样时间 - 微分时间
loopNum = 50      

class controlMethod():      # 求解桑塔格控制率的时候，使用了机理模型中获得的导数
    def __init__(self):
        # 稳定点数值
        self.CA1s = 1.954
        self.CA2s = 1.954
        ...
        self.model =torch.load(self.PATH, map_location=device)
    ...
    def calObjective(self, controlSeq, seqSize, initialConditions, initialControls):
        ...
        for i in range(0, seqSize):
            ...
            sum = sum + (xTemp.T.dot(self.Q).dot(xTemp) + u1Temp.T.dot(self.R).dot(u1Temp) + \
                u2Temp.T.dot(self.R).dot(u2Temp)) * sampleInterval
    ...
    def getConstraintJacVec(self, controlSeq, seqNum, seqSize, initialConditions, initialControls, returnNum, modelType='NN'):
        ...
        #     fnn_ = (originPointNow - oringinPoint)/stepLen
        #     jacVec[j] = (originPointNow - oringinPoint)/stepLen
        
        for i in range(seqSize):
            jacVec[4*i] = q/V
            jacVec[4*i+1] = 1/(rho*Cp*V)
            jacVec[4*i+2] = q/V
            jacVec[4*i+3] = 1/(rho*Cp*V)

        return jacVec

    def getConstraintJacVec_backup(self, controlSeq, seqNum, seqSize, initialConditions, initialControls, returnNum, modelType='NN'):
        #                   self, controlSeq, seqNumNow, seqSize, initialConditions, initialControls, returnNum
        # seqNum       当前预测时间步
        # seqSize      MPC预测窗口大小
        # initialConditions 当前时刻的未开始MPC的状态
    ...
    def CentralizeLMPC(self, initialConditions, initialControls, seqSize):
        ...
        # def h(x, lagrange, obj_factor, out):
        #     varNum = len(x)
        #     P = np.zeros([varNum, varNum])
        #     for i in range(0,varNum,2):
        #         P[i:i+2, i:i+2] = self.P
        #     iterateValue = np.zeros([varNum, varNum])
        #     constrainNum = len(lagrange)
        #     for i in range(0,constrainNum,2):
        #         iterateValue = iterateValue + lagrange[i] * np.diag(self.getConstraintJacVec(x, int(np.floor(i/2)), seqSize, initialConditions, initialControls,0, self.choice))
        #         iterateValue = iterateValue + lagrange[i+1] * np.diag(self.getConstraintJacVec(x, int(np.floor(i/2)), seqSize, initialConditions, initialControls,1, self.choice))

        #     matrixUsed = 2*P*obj_factor + iterateValue
        #     for i in range(0, self.seqSize):
        #         out[4*i] = matrixUsed[i,i]
        #         out[4*i+1] = matrixUsed[i, i+1]
        #         out[4*i+2] = matrixUsed[i+1, i]
        #         out[4*i+3] = matrixUsed[i+1,i+1]
        #     # out[()]= np.full((2*varNum,), 2*P*obj_factor + iterateValue)
        #     return out
